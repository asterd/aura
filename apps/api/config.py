...
class Settings(BaseSettings):
    ...
    litellm_master_key: SecretStr = Field(default=SecretStr("sk-local-master-key"), alias="LITELLM_MASTER_KEY")
    litellm_proxy_key_sync_enabled: bool = Field(default=True, alias="LITELLM_PROXY_KEY_SYNC_ENABLED")
    litellm_proxy_key_duration: str = Field(default="permanent", alias="LITELLM_PROXY_KEY_DURATION")
    local_auth_jwt_secret: SecretStr = Field(default=SecretStr("aura-local-dev-secret"), alias="LOCAL_AUTH_JWT_SECRET")
    local_auth_jwt_issuer: str = Field(default="aura-local", alias="LOCAL_AUTH_JWT_ISSUER")
    local_auth_audience: str = Field(default="api://default", alias="LOCAL_AUTH_AUDIENCE")
    bootstrap_token: SecretStr = Field(default=SecretStr("aura-bootstrap-dev-token"), alias="AURA_BOOTSTRAP_TOKEN")
